...
if __name__ == "__main__":
    ...
    epoch = 5
    history = model.fit(
        X_train_scaled, y_train, epochs=epoch, batch_size=32, validation_split=0.2
    )
    ...
    print(
        f"Среднее время инференса оригинальной модели: {inference_time_original:.5f} секунд."
    )
    ...
    for _ in range(cnt_reload):
        onnx_predictions = session.run(
            None, {input_name: X_test_scaled.astype(np.float32)}
        )
